...
def buildScore(chords, melody, length, t, swing, midi, isFourFour):
    ...
    # make parts for each instrument, drums optionally
    ...
    ## Fill up all parts to the longest length
    longestLength = max(
...
def getTrades(chords, melody, length):
    '''
    Generates the trades for the piano and horn solos.
    The trades are generated by alternating between the two instruments.
    '''
    # piano solo, length of each trade
    ...
    ## loop thru notes in the solo and assign to horn or piano trade
    for n in solo:
        if curOffset >= curLength: # jump to next trade
            curLengthIdx += 1
            curLength += lengths[curLengthIdx]
        if curLengthIdx % 2 == 0: # piano time
            pianoTrades.append(n)
            hornTrades.append(note.Rest(quarterLength=n.quarterLength))
        else: 
            hornTrades.append(n) # horn time
...
def fillUpToLength(part, length):
    """
    Adds rests to part until it reaches the given length.
    """
    while part.quarterLength < length:
        part.append(note.Rest())
    if part.quarterLength > length: # remove remainder
...
def getHornPart(chords, melody, length, t, swung, midi, hornTrades):
    """
    Generates the horn part based on the given chords and melody.
    Part contains the head, 3 solos, and the head again.
    Part will play the melody, solo is generated with getSolo()
    """
...
def getBassPart(chords, melody, length, t, swung):
    """
    The bass part is based on chords from the sheet music
    and is generated using getBassPattern().
    Patterns will be made and play throughout head and 3 solos, and end head
...
def getBassPattern(chords, length):
    ...
    ## For each quarter note offset
    ## Generate a note that is in chord, using quality-based markov chain
    for offset in range(int(length)):
        # Looking at a new chord: its the first one, or the next chord just started
        ...
        if r < 0.4 and (n.pitch.midi - curRoot) % 12 not in chordNotes: 
            for i in shuffledChordNotes: # choose close chord note to resolve to
                if abs(((n.pitch.midi - curRoot) % 12) - i) == 1:
                    resolveNote = note.Note(i + curRoot - 12)
                    break
                    
            if resolveNote is None: # resolve 2 away as backup
...
def makeMarkovChain(chord, scale):
    """
    Dynamically creates a Markov chain for the given chord and scale. 
    ...
    # calculate probabilities using bucket scoring
    # marbles are added to buckets to increase likeliness based on note/chord relationships
    mc = {}
    for prev in scale:
        row = {}
        buckets = [0] * len(scale) 
        for i, choice in enumerate(scale): 
            ...
            if choice in chord and prev not in chord: # notes in the chord have a higher chance of being selected
                buckets[i] += 1
            if choice not in chord and prev in chord: # note is not in chord, but previous note is in chord
                buckets[i] += 1
            if choice == prev:
                if i != 0:
                    buckets[(i - 1)] += 1 # note before 
                buckets[i] == 0
                if i != len(scale) - 1:
                    buckets[(i + 1)] += 1 # note after 
        buckets[scale.index(prev)] == 0
        bucketrates = [0.95 * i / sum(buckets) for i in buckets] # other note probs fills up 95% of each row
        
        # 5% chance of repeated note
        for i, choice in enumerate(scale):
            if prev == choice: 
                row[choice] = 0.05 
            else:
...
def getPianoPart(chords, length, t, swung):
    """
    Generates the piano accomp part based on the given chords and length.
    The part is generated using getPianoPattern()
    Patterns will be made and play throughout head and 3 solos, and end head
...
def getPianoPattern(chords, length):
    ...
    Rhythms are generated by a Monte Carlo method, 
    Chord tones are combined and mapped into piano accomp, along with rests
    ...
    ## loop thru each quarter note offset
    ## Determine length of each chord, and generate a rhythm for it
    for offset in range(int(length)):
        # Looking at a new chord: its the first one, or the next chord just started
        if chordIndex == -1 or (chordIndex != len(chords) - 1 and chords[chordIndex + 1]["offset"] <= offset):
            chordIndex += 1
            ...
            # get lenght of the chord in offsets
            ...
            # generate numerical rhythm over the chord length, map notes/rests to it 
        ...
        # Else, iterate until next chord starts to find length

    # copy all notes/rests to the total piano pattern
    for el in totalmap:
        #Flatten notes
...
def getRhythms(length): 
    """
    Generates a list of rhythms based on the given length.
    The rhythms are generated using a Monte Carlo method, 
    where the probabilities of each rhythm are based short/long length of the chord.
    """
    currentLength = 0
    rhythms = []
    # choose numerical rhythms that add up to the length of the chord
    while currentLength != length:
        weights = SHORT_LENGTH_WEIGHTS if length < 6 else LONG_LENGTH_WEIGHTS
        currChoice = random.choices(LENGTH_OPTIONS, weights=weights, k=1)[0]

        # ensure current length isn't too long, and is unique from prev 2 choices
        if currChoice <= length - currentLength and \
            (len(rhythms) < 2 or rhythms[-1] != currChoice or rhythms[-1] != rhythms[-2] or currentLength == length - 0.5):
                rhythms.append(currChoice)
                currentLength += currChoice
    return rhythms

def mapRhythms(notes, root, rhythms, startingOffset):
    """
    Assigns each numerical rhythm to a note or rest, assigning offets to each element
    Notes are combined chord tones, each have inclusion probabilities based on dist from root.
    """

    comp = []
    currentOffset = startingOffset # offset of the entire part, not just of the chord lenght
    addRest = False
    notesInChord = [note.Note(n + root) for n in notes]
    currChordAmount = 0 # num chords (attacks) played by accomp

    ## loop through each rhythm and assign a note/rest to it
    for i, rhythm in enumerate(rhythms):
        # ratios for notes and rests are based on the length of the chord
        chordRate = 0.3 if len(rhythms) < 4 else 0.6 # ensure higher notes ratio played over longer sections
        ...
        ## assign probs for intervals from root
        other = 0.5
        incl_probs = [0.15, other, other, 0.9, 0.9, other, 0.25, other, other, 0.9, other, other]
        if not addRest:
            chordNotes = []
            while len(chordNotes) < 2: # at least 2 notes in a chord
                for n in notesInChord:
                    noteIntervalFromRoot = abs((n.pitch.midi - root) % 12)
                    ratio = random.random()
                    if ratio < incl_probs[noteIntervalFromRoot] and n not in chordNotes:
                        chordNotes.append(n)

        ## tie logic for rhythms that will overlap the bar
        if currentOffset + rhythm > 4:
            # break up ryhthm into 2 notes/rests
        ...
        else: # assign a note/rest to the rhythm
...
def getPianoSoloPart(chords, melody, length, t, swung, pianoTrades):
    """
    Generates the piano solo part based on the given chords and melody.
    The part contains the head, piano solo, trading fours, and the head again.
    This part just plays during the piano solo, which is pitched up an octave
    """
    pianoPart = stream.Part()
    pianoPart.append(instrument.Piano())
...
def get_drums(length, t, midi):
    """
    Generates a drum part based on the given length and if midi is selected
    The part contains a ride pattern, hihat pattern, bass pattern, and snare pattern.
    """
    ...
    else: # Unpitched percussion for sheet music
    ...
    ## patterns for each type of drum
    ridePattern = [
    ...
    ## append the patterns to the parts
    ...
    if t is not None:
        snarePart.append(t)
    ## cover all 5 sections of the song
